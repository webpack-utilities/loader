...
function stringifyRequest (loader, request) {
  const splitted = request.split("!")

  const context = loader.context || (loader.options && loader.options.context)

  return JSON.stringify(splitted.map((part) => {
    // First, separate singlePath from query,
    // because the query might contain paths again
    ...
    if (isAbsolutePath(singlePath) && context) {
      ...
      if(isAbsolutePath(singlePath)) {
        ...
        // In this case, we leave the path platform-specific
        // without replacing any separators
      ...
      if(isRelativePath(singlePath) === false) {
        ...
        // otherwise it would be a request into the modules directory
        // (like node_modules)
